...
public class SampleViewModel implements Initializable {
	...
	int EmailThreshold, TimeToSendEmail, TimeToRefreshTable;
	...
	public Node createPage(int pageIndex) {
		...
		int toIndex = Math.min(fromIndex + 5,productsrows.size());
	...
	public void SearchBtn() {
		...
		ObservableList<SampleViewModel.Product> tableItems = CollectionTable.getItems();
		ObservableList<SampleViewModel.Product> FilteredtableItems = FXCollections.observableArrayList();
		for (Product curr : tableItems) {
	...
	void SaveRecipientEmail(ActionEvent event)  {
		...
		System.out.println(EmailsToSendList);
		for (String to : EmailsToSendList)
		ServiceFacade.Email(to, "david23");
	}
